...
| `condition` | Walking condition (1 = unbraced, 2= knee brace, 3 = ankle brace) |
| `replication` | Trial number per subject |
| `leg` | 1 = left, 2 = right |
| `joint` | 1 = ankle, 2 = knee, 3 = hip |
| `time` | 0 = 0% gait cycle, …, 100 = 100% gait cycle |
...
- Computes velocity, acceleration and jerk profiles.  
- Explores gait cycle consistency and variation under braced vs unbraced conditions.
- Detects short-term or fine-grained irregularities in gait motion.  
- Outputs: Anomaly detection and gait health score per instance.

### 2. **Macro-Level Analysis**
- Uses cycle-wise aggregated data for machine learning.
- Captures overall gait deviations and movement pattern classification.
